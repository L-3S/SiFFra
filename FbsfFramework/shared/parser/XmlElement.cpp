...
void Element::handleElement(XmlParser *parser, const char *childName,
                            int isEmptyElement,const QDomElement& xmlElem) {
...
void Element::printElement(int indent) {
    ...
    qDebug() << ERROR_INFO<< indentS.c_str() << XmlParser::elmNames[type];
    for (std::map<XmlParser::Att, char *>::const_iterator it = attributes.begin(); it != attributes.end(); ++it)
    {
        qDebug() <<ERROR_INFO << indentS.c_str() << XmlParser::attNames[it->first] << "=" <<it->second;
    }
}
...
void ListElement::handleElement(XmlParser *parser, const char *childName,
                                int isEmptyElement,const QDomElement& xmlElem) {
    XmlParser::Elm childType = parser->checkElement(childName);
    if (childType == XmlParser::elm_Item) {
        Element *item = new Element;
        item->type = childType;
        parser->parseElementAttributes(item,xmlElem);
        if (!isEmptyElement) {
...
void Unit::handleElement(XmlParser *parser, const char *childName,
                         int isEmptyElement,const QDomElement& xmlElem) {
    XmlParser::Elm childType = parser->checkElement(childName);
    if (childType == XmlParser::elm_BaseUnit) {
        baseUnit = new Element;
        baseUnit->type = childType;
        parser->parseElementAttributes(baseUnit,xmlElem);
        if (!isEmptyElement) {
            parser->parseEndElement();
        }
        ...
        parser->parseElementAttributes(displayUnit,xmlElem);
...
void SimpleType::handleElement(XmlParser *parser, const char *childName,
                               int isEmptyElement,const QDomElement& xmlElem) {
    XmlParser::Elm childType = parser->checkElement(childName);
    switch (childType) {
        ...
        case XmlParser::elm_String: {
            ...
            parser->parseElementAttributes(typeSpec,xmlElem);
            if (!isEmptyElement) {
                parser->parseEndElement();
            }
            break;
        ...
        case XmlParser::elm_Enumeration: {
            ...
            parser->parseElementAttributes(typeSpec,xmlElem);
            if (!isEmptyElement) {
                parser->parseChildElements(typeSpec,xmlElem);
            }
            break;
        }
        default: {
...
void Component::handleElement(XmlParser *parser, const char *childName,
                              int isEmptyElement,const QDomElement& xmlElem) {
    XmlParser::Elm childType = parser->checkElement(childName);
    if (childType == XmlParser::elm_SourceFiles) {
        if (!isEmptyElement) {
            parser->parseChildElements(this,xmlElem);
        }
    } else if (childType == XmlParser::elm_File) {
        Element *sourceFile = new Element;
        sourceFile->type = childType;
        parser->parseElementAttributes(sourceFile,xmlElem);
        if (!isEmptyElement) {
            parser->parseEndElement();
        }
...
void ScalarVariable::handleElement(XmlParser *parser, const char *childName,
                                   int isEmptyElement,const QDomElement& xmlElem) {
    XmlParser::Elm childType = parser->checkElement(childName);
    switch (childType) {
        case XmlParser::elm_Real:
        ...
        case XmlParser::elm_Annotations: {
            ...
            if (!isEmptyElement) parser->parseChildElements(this,xmlElem);
            break;
        }
        case XmlParser::elm_Tool: {
            Element *tool = new Element;
            tool->type = childType;
            parser->parseElementAttributes(tool,xmlElem);
            if (!isEmptyElement) {
...
void ModelStructure::handleElement(XmlParser *parser, const char *childName,
                                   int isEmptyElement,const QDomElement& xmlElem) {
    XmlParser::Elm childType = parser->checkElement(childName);
    switch (childType) {
    ...
    case XmlParser::elm_Unknown:
        {
            ...
            parser->parseElementAttributes(unknown,xmlElem);
            if (!isEmptyElement) {
                parser->parseEndElement();
            }
...
void ModelDescription::handleElement(XmlParser *parser,
    ...
    case XmlParser::elm_CoSimulation:
        {
            ...
            parser->parseElementAttributes(coSimulation,xmlElem);
            if (!isEmptyElement) {
                parser->parseChildElements(coSimulation,xmlElem);
            }
            break;
        }
    case XmlParser::elm_ModelExchange:
        {
            ...
            parser->parseElementAttributes(modelExchange,xmlElem);
            if (!isEmptyElement) {
                parser->parseChildElements(modelExchange,xmlElem);
            }
            break;
        }
    ...
    case XmlParser::elm_Unit:
        {
            Unit *unit = new Unit;
            unit->type = childType;
            parser->parseElementAttributes(unit,xmlElem);
            if (!isEmptyElement) {
                parser->parseChildElements(unit,xmlElem);
            }
    ...
    case XmlParser::elm_SimpleType:
        {
            ...
            type->type = childType;
            parser->parseElementAttributes(type,xmlElem);
            if (!isEmptyElement) {
                parser->parseChildElements(type,xmlElem);
            }
    ...
    case XmlParser::elm_DefaultExperiment:
        {
            ...
            parser->parseElementAttributes(defaultExperiment,xmlElem);
            if (!isEmptyElement) {
                parser->parseEndElement();
            }
            break;
    ...
    case XmlParser::elm_Category:
        {
            ...
            parser->parseElementAttributes(category,xmlElem);
            if (!isEmptyElement) {
                parser->parseEndElement();
            }
    ...
    case XmlParser::elm_ScalarVariable:
        {
            ...
            parser->parseElementAttributes(variable,xmlElem);
            if (!isEmptyElement) {
                parser->parseChildElements(variable,xmlElem);
            }
    ...
    case XmlParser::elm_ModelStructure:
        {
            ...
            parser->parseElementAttributes(modelStructure,xmlElem);
            if (!isEmptyElement) {
                parser->parseChildElements(modelStructure,xmlElem);
            }
            break;
        }
    default:
