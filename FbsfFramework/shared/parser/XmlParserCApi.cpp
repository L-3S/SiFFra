...
int getUnitDefinitionsSize(ModelDescription *md) {
    return (int)md->unitDefinitions.size();
}
...
int getTypeDefinitionsSize(ModelDescription *md) {
    return (int)md->typeDefinitions.size();
}
...
int getLogCategoriesSize(ModelDescription *md) {
    return (int)md->logCategories.size();
}
...
int getVendorAnnotationsSize(ModelDescription *md) {
    return (int)md->vendorAnnotations.size();
}
...
int getScalarVariableSize(ModelDescription *md) {
    return (int)md->modelVariables.size();
}
...
int getDerivativesSize(ModelStructure *ms) {
    return (int)ms->derivatives.size();
}
...
int getDiscreteStates(ModelStructure *ms) {
    return (int)ms->discreteStates.size();
}
...
int getInitialUnknowns(ModelStructure *ms) {
    return (int)ms->initialUnknowns.size();
}
...
int getAnnotationsSize(ScalarVariable *sv) {
    return (int)sv->annotations.size();
}
...
int getDisplayUnitsSize(Unit *u) {
    return (int)u->displayUnits.size();
}
...
const char *getAttributeFromUnitOrDeclaredUnit(ModelDescription *md,ScalarVariable *sv)
{
    const char *unitName = sv->typeSpec->getAttributeValue(XmlParser::att_unit);
    if (unitName==NULL)
    {
        const char* declaredType=sv->typeSpec->getAttributeValue(XmlParser::att_declaredType);
        if(declaredType!=NULL)
        {
            SimpleType* st=getSimpleType(md,declaredType);
            if(st!=NULL)
                return st->typeSpec->getAttributeValue(XmlParser::att_unit);
        }
        return "";
    }
    return unitName;
