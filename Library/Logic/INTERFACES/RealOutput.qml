...
PartialSI
{
    ...
    function initialize()
    {
        console.log("Initialisation de ", nodeName, " mode simuMpc = ", simuMpc);
    ...
    function compute()
    {
        if (!simuMpc) {
            ...
            if (index < oldPastEnd + timeShift) saveOldpast[index] = input.value
        }
    }
}
