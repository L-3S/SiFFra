...
int TestLoadSuccess(int ac, char **av, QString& mess)
{
    ...
    // Instantiation
    void *pComp = FbsfInstantiate("simul_NoGr.xml", ac, av);
    if (!pComp)
    {
...
int TestSimpleSimuFMU(int ac, char **av, QString& mess)
{
    ...
    void *pComp = FbsfInstantiate("TestFMU_Crash_UnRes.xml", ac, av);
    if(!pComp || FbsfGetStatus(pComp, &st)==Failure || st==FbsfUninitialized)
    {
...
//            FbsfSaveState(pComp);
    ...
    if(FbsfGetStatus(pComp, &st)==Failure || st ==FbsfFailedStep){
        mess =  "ERROR: TestSaveRestore: 7-Restore was unsuccessful";
        return 1;
    };

    if(FbsfDoStep(pComp, timeOutCPUs) == Failure || FbsfGetStatus(pComp, &st)==Failure){
        mess =  "ERROR: TestSaveRestore: 8-Timestepping after failure was unsuccessful";
        return 1;
    };

    if(FbsfGetRealData(pComp, simuTimeString, &ZEValTime) == Failure || abs(ZEValTime - 2 * timestep) > 1.e-6
            || FbsfGetRealData(pComp, TimeInt, &ZEFMUTimeInt) == Failure || FbsfGetRealData(pComp, TimeValFMU, &ZEFMUTime) == Failure){
        mess =  "ERROR: TestSaveRestore: 9-Read of updated values from simulation after failure was unsuccessful";
        return 1;
    };

    if((ZEValTimeSave != ZEValTime) || (ZEFMUTimeIntSave != ZEFMUTimeInt) || (ZEFMUTimeSave != ZEFMUTime)){
        mess =  "ERROR: TestSaveRestore: 10-Updated values from simulation after failure are not compliant";
        return 1;
    };

    if(FbsfTerminate(pComp)==Failure || FbsfGetStatus(pComp, &st)==Failure || st!=FbsfTerminated)
    {
        mess =  "ERROR: TestSaveRestore: 11-Termination step was unsuccessful";
        return 1;
    }
    if(FbsfFreeInstance(&pComp)==Failure || pComp)
    {
...
int main(int ac, char **av) {
    QString const defaultSimuFile = "TestFMU_Crash_UnRes.xml";
    void *comp = nullptr;
    std::string cmd;
    QString mess;
    cout << "command:";
    while (getline(cin, cmd)){
        ...
        if (cmd == "t1") {
            ...
            std::cout << "SUCCESS: TestLoadSuccess \n";
            return 0;
        }
        if (cmd == "t1b") {
            ...
            std::cout << "SUCCESS: TestLoadFailure \n";
            return 0;
        }
        if (cmd == "t2") {
            ...
            std::cout << "SUCCESS: TestSimpleSimu \n";
        ...
        if (cmd == "t2b") {
            ...
            std::cout << "SUCCESS: TestSimpleSimuFMU \n";
            return 0;
        }
        if (cmd == "t3") {
            ...
            std::cout << "SUCCESS: TestWallClock \n";
            return 0;
        }
        if (cmd == "t4") {
            ...
            std::cout << "SUCCESS: TestSaveRestore \n";
            return 0;
        }
        if (cmd == "t5") {
            ...
            std::cout << "SUCCESS: TestRestoreWithCorr \n";
            return 0;
        }
        if (cmd == "data") {
